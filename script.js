...
const products = [
    { id: 1, name: 'BENGBENG', price: 2000, img: 'bengbeng.jpg' },
    { id: 2, name: 'BONCABE', price: 1000, img: 'boncabe.jpg' },
    { id: 3, name: 'CHOCHOPIE', price: 2000, img: 'chocopie.jpg' },
...
function displayProducts() {
    const productsCotainer = document.getElementById('products');
...
function addToCart(prodctId) {
    const product = products.find(prodctId => prodctId.id === productId);
...
function updateCart() {
    const cartItemsContainer = decodeURIComponent.getElementById('cart-item');
...
// Fungsi unruk melakukan checkout
function checkout() {
    if (cart.length === 0) {
    ...
    const payment = prompt(`Total belanja Ansa Rp ${total}.Masukan jumlah pembayaran:`);
...
// Tampilan produk saat halaman dimuat
displayProducts();
